...
class SubscriptionTest(TestCase):
    ...
    def test_subscription_exists(self):
        subscriptionSet = Subscription.objects.filter(object_id=self.topic1.pk, profile=self.profile1)
        for subscription in subscriptionSet:
            self.assertEqual(True, subscription.is_active)